...
### Lot 8/8 — Technologie, données, IA (contrôle de couverture) + réconciliation finale ✅ COMMITÉ (2026-07-11)

**⚠️ Correction du lot 1** : une réconciliation programmatique des 8 thèmes (IDs actifs réels vs. IDs effectivement revus par lot) a révélé que le lot 1 (ECONOMY) contenait une erreur d'extraction antérieure à la fenêtre de contexte visible de cette session — 3 IDs faussement comptés comme « conformes » (en réalité des doublons : `ECO_16`, `ECO_21`, `ECO_22`) et 2 questions actives jamais examinées (`ECO_26` CORE, `ECO_27`). Isolé au seul lot 1 — lots 2 à 7 confirmés exacts. `ECO_26` revue : déjà bien construite, aucun changement. `ECO_24` et `ECO_27` se sont révélées être des quasi-doublons inter-thèmes non corrigés de `GLO_10` et `GLO_6` (déjà signalés dans `02-question-audit.md` de l'audit d'origine sans avoir été traités) — recentrées sur des nuances distinctes (adéquation du taux de 15% pour `ECO_24` ; droits compensateurs ciblés pour `ECO_27`), polarité vérifiée inchangée. Détail complet et table de correction : [17-editorial-batches-synthesis.md](17-editorial-batches-synthesis.md#lot-88--technologie-données-ia-contrôle-de-couverture--réconciliation-finale).

**Couverture technologie/IA** : constat confirmé indépendamment (déjà identifié dans `03-topic-coverage.md` de l'audit d'origine) — aucune des 200 questions ne porte au premier plan sur la régulation de l'IA, la décision algorithmique, ou la collecte de données par les entreprises privées ; 8 questions touchent le numérique en angle secondaire d'un autre sujet (surveillance étatique, pouvoirs policiers, droit des plateformes, concurrence). Conformément au cadrage de la mission (audit éditorial, pas extension du corpus), **ce lot documente le manque et propose 6 questions candidates sans les appliquer** — ajouter un thème nécessiterait des changements d'architecture (nouveau préfixe, recalcul des axes, mise à jour de la mention « 8 thèmes » dans l'UI) dépassant le périmètre éditorial. Deux autres angles morts signalés par l'audit d'origine (égalité F/H, accountability policière — disparus d'un brouillon antérieur sans explication documentée) sont mentionnés pour mémoire, hors périmètre de correction.

**Commits** :
- **`1968eee`** — *fix: reconcile ECO_24/ECO_27, close two inter-theme redundancies*
- Commit documentaire à suivre (ce fichier + synthèse lot 8 + propositions technologie/IA).

**Tests** : régression ×4 PASS (107 questions direction=1, inchangé), JSON valide (200 entrées), lint ECONOMY (1 faux positif déjà connu, ECO_20). Build OK. Vérification navigateur réelle : `priorityOrder` positionné sur Économie en premier, mode Approfondi (64 questions), ECO_1 *(CORE)* confirmé en première question, rendu propre sur mobile 375×812.

## Bilan de la mission des 8 lots (162/162 questions actives auditées, 100 %)

**26 questions modifiées** au total à travers les 8 thèmes. **3 vrais changements méthodologiques avec bascule de polarité ou redondance inter-thème corrigée en profondeur** documentés avec vérification de direction dédiée (`ECO_13` — seule vraie bascule de `DIRECTION_MAP` — , `ENV_8`, `SOC_21`, plus `ECO_24`/`ECO_27` au lot 8). **3 doublons inter-thèmes** corrigés (`ECO_24`/`GLO_10`, `ECO_27`/`GLO_6`, `SOC_21`/`SEC_9`) — un angle mort du système `isDuplicate`, qui n'opère qu'au sein d'un même thème. **1 bug technique significatif** découvert et corrigé en cours de route (`questionHints.js` désynchronisé sur 10 entrées, affectant l'affichage réel indépendamment du travail éditorial — dont 2 corrections déjà committées mais invisibles aux utilisateurs avant la découverte). **1 erreur de processus** (lot 1) découverte et corrigée en toute transparence plutôt que dissimulée. **1 angle mort thématique majeur** (technologie/IA/données) documenté avec 6 questions candidates proposées mais non appliquées ; 2 angles morts mineurs signalés pour mémoire (égalité F/H, accountability policière).
